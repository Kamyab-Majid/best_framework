...
class RedshiftConnector:
    ...
    def __init__(self, **kwargs) -> None:
        ...
        self.cluster_identifier = kwargs["cluster_identifier"] \
            if "cluster_identifier" in kwargs and kwargs["cluster_identifier"] else None
        self.profile = kwargs["profile"] if "profile" in kwargs and kwargs["profile"] else 'default'
        self.autocommit = kwargs["autocommit"] \
            if "autocommit" in kwargs and kwargs["autocommit"] else False
    ...
    def create_connection_using_IAM_credentials(self) -> Connection:
        ...
        Returns:
            Connection: A Connection object associated with the specified Amazon Redshift cluster
        """
        if self.database and self.db_user and self.password and self.user \
            and self.cluster_identifier:
            return redshift_connector.connect(
                iam=self.iam,
                database=self.database,
    ...
    def redshift_execute_select(self, table: str, columns: str = "*") -> tuple:
        ...
        Returns:
            tuple: Result of data displayed in a tuple of lists (rows of data).
            If data is empty, it will be logged as an info for user's awareness.
        """
        if self.connection:
    ...
    def redshift_execute_multiple_queries(self, table: str, query_statements: list) -> tuple:
        ...
        Returns:
            tuple: Result of final data displayed in a tuple of lists (rows of data)
    ...
    def redshift_execute_insert(
        ...
        if source_table:
            query_statements.append(
                f"INSERT INTO {table} (SELECT {source_columns} FROM {source_table})")
        if values:
            query_statements.append(f"INSERT INTO {table} VALUES {values}")

        return self.redshift_execute_multiple_queries(
            table=table, query_statements=query_statements)


    def redshift_execute_delete(
        self, table: str, reference_tables: str = None, conditions: str = None
    ) -> tuple:
        """
        ...
        if reference_tables:
            if conditions:
                query_statements.append(
                    f"DELETE FROM {table} USING {reference_tables} WHERE {conditions}")
            else:
                raise RuntimeError(
                    "Conditions argument needs to be specified for reference tables.")
        else:
            if conditions:
