# pylint: disable=redefined-outer-name, missing-module-docstring, missing-function-docstring, unnecessary-dunder-call, unused-wildcard-import, wildcard-import, singleton-comparison
...
def test_delegate_pandas_df_on_date_column_return_correct_value(
    mock_pandas_df, desired_values
):
    new_df = delegate_df(df=mock_pandas_df, date_column="date", new_format="%B %d, %Y")
...
def test_delegate_pandas_df_on_string_date_column_return_pandas_df(mock_pandas_df):
    new_df = delegate_df(
        df=mock_pandas_df, date_column="string_date", new_format="%B %d, %Y"
    )
    assert isinstance(new_df, pandas_data_frame)


def test_delegate_pandas_df_on_string_date_column_return_correct_value(
    mock_pandas_df, desired_values
):
    new_df = delegate_df(
        df=mock_pandas_df, date_column="string_date", new_format="%B %d, %Y"
...
def test_delegate_pandas_df_on_non_date_column_should_raise_value_error(mock_pandas_df):
    with pytest.raises(ValueError):
        delegate_df(
            df=mock_pandas_df, date_column="letter_code", new_format="%B %d, %Y"
        )
...
def test_delegate_spark_df_on_date_column_return_spark_df(mock_spark_df):
    new_df = delegate_df(
        df=mock_spark_df, date_column="date", new_format="MMMM dd, yyy"
    )
    assert isinstance(new_df, spark_data_frame)


def test_delegate_spark_df_on_date_column_return_correct_value(
    mock_spark_df, desired_values
):
    new_df = delegate_df(
        df=mock_spark_df, date_column="date", new_format="MMMM dd, yyy"
    )
    delegated_values = delegated_spark_df_res_list(
        data_frame=new_df, delegated_col="date"
    )
    check_cond = delegated_values == desired_values
    assert check_cond == True
...
def test_delegate_spark_df_on_string_date_column_return_spark_df(mock_spark_df):
    new_df = delegate_df(
        df=mock_spark_df, date_column="string_date", new_format="MMMM dd, yyy"
    )
    assert isinstance(new_df, spark_data_frame)


def test_delegate_spark_df_on_string_date_column_return_correct_value(
    mock_spark_df, desired_values
):
    new_df = delegate_df(
        df=mock_spark_df, date_column="string_date", new_format="MMMM dd, yyy"
    ...
    delegated_values = delegated_spark_df_res_list(
        data_frame=new_df, delegated_col="string_date"
    )
    check_cond = delegated_values == desired_values
    assert check_cond == True
...
def test_delegate_spark_df_on_non_date_column_should_raise_value_error(mock_spark_df):
    with pytest.raises(ValueError):
        delegate_df(
            df=mock_spark_df, date_column="letter_code", new_format="MMMM dd, yyy"
        )
...
def test_delegate_list_date_obj_return_correct_value(
    mock_list_date_obj, desired_values
):
    new_list_obj = delegate_df(df=mock_list_date_obj, new_format="%B %d, %Y")
...
def test_delegate_dict_date_obj_return_correct_value(
    mock_dict_date_obj, desired_dict_values
):
    new_dict_obj = delegate_df(df=mock_dict_date_obj, new_format="%B %d, %Y")
