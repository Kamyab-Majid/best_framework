...
class ArgParser(argparse.ArgumentParser):
    def __init__(self,*args, **kwargs) -> None:
        """ This is an inherited class from argparse library with additional methods to add and parse custom arguments.
    ...
    def add_custom_arguments(self, args: dict) -> None:
        """This method takes a dictionary of custom arguments and add to the system to define how those arguments are taken on the command line to be parsed
        Args:
            args (dict): a list of arguments in dictionary format with key is the argument name and value is more details for the command-line argument
            in terms of action/default value/ required (if applicable)

        Raises:
            ValueError: when custom input arguments are not sufficient with information (not in dictionary format)
        """
        if not isinstance(args, dict):
            error_msg = 'Arguments passed in needs to be in dictionary format with key-value pair(s)'
    ...
    def add_custom_argument(self, arg_name: str, arg_value: Any) -> None:
        """This method is a sub-step in the loop of adding custom arguments. This takes both argument name and its value detail to define the command-line argument

        Args:
            arg_name (str): name of the custom argument to be added
            arg_value (Any): details (if any) of the custom argument. It could be a boolean (to store argument as true/false), a default value, a dictionary with key acts as a
            parameter of the add_argument() method (e.g. {required: True}), or it could be None (adding the argument without any specifications)
        """
        if arg_value:
    ...
    def parse(self, args: list[str] = None) -> dict:
        """This method convert custom argument strings to object and assign them as attributes of a result dictionary

        Args:
            args (list[str], optional): list of argument/option strings that wish to be parsed. Defaults to None (value is taken from sys.argv).

        Raises:
            ValueError: when there's no arguments passed before parsing

        Returns:
            dict: dictionary of results with argument name being the key and its parsed value being the value
        """
        parsed_dict = None
