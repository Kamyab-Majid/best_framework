import sys
import pytest
from pynutrien.env.args import ArgParser
...
def test_when_args_are_parsed_then_return_dict(arg_parser):
    ...
    parsed_dict = arg_parser.parse(
        ["--debug_run", "--input_connector", "a", "--debug_level", "b", "--value", "c"]
    )
    assert isinstance(parsed_dict, dict) == True
...
def test_when_args_should_be_parsed_as_bool_only(arg_parser):
    ...
    assert parsed_dict["debug_run"] == True
...
def test_when_args_are_parsed_as_none(arg_parser):
    ...
    assert parsed_dict["debug_run"] == None
