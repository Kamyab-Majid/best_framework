...
from pynutrien.utility.standardize_column_names import standardize_column_names

@pytest.fixture
def valid_pandas_df():
    return pd.read_csv("tests/std_column_names_test_files/bigmac.csv")
...
def valid_spark_df(spark):
    return spark.read.format("csv").option("header", True).load(
        "tests/std_column_names_test_files/bigmac.csv")
...
def test_pandas_df_with_no_header_should_raise_runtime_error(
    pandas_df_with_no_headers):
    with pytest.raises(RuntimeError):
        standardize_column_names(pandas_df_with_no_headers)

def test_pandas_df_with_integer_headers_should_standardize_col_name_correctly(
    pandas_df_with_integer_headers):
...
def test_spark_df_with_no_header_should_raise_runtime_error(
    spark_df_with_no_headers):
    with pytest.raises(RuntimeError):
        standardize_column_names(spark_df_with_no_headers)

def test_spark_df_with_integer_headers_should_standardize_col_name_correctly(
    spark_df_with_integer_headers):
