...
from pynutrien.aws.redshift.connector import RedshiftConnector
...
def test_creating_connection_with_wrong_credentials_or_cluster_config_should_raise_runtime_error(
    wrong_cluster_config
):
    with pytest.raises(RuntimeError):
        RedshiftConnector(
            host=wrong_cluster_config['host']
        )

    with pytest.raises(RuntimeError):
        RedshiftConnector(
            iam=True,
            user=wrong_cluster_config['user'],
            password=wrong_cluster_config['password']
        )

def test_creating_connection_to_non_accessible_redshift_cluser_should_raise_runtime_error(non_accessible_cluster_config):
    with pytest.raises(RuntimeError):
        RedshiftConnector(
...
def test_querying_select_from_redshift_should_return_correct_data(
...
def test_querying_select_from_a_non_existing_table_or_column_should_raise_runtime_error(
    conn):
    with pytest.raises(RuntimeError):
        conn.redshift_execute_select(table='non_existing')

# test_redshift_execute_insert
def test_querying_insert_to_redshift_from_a_source_table(
    conn, correct_return_data_when_select_query):
...
def test_querying_insert_to_redshift_using_specified_custom_values(
    conn):
    data_after_inserted = conn.redshift_execute_insert(
        table=staged_table_name,
        values="\
...
def test_querying_insert_with_missing_argunments_should_raise_runtime_error(
    conn):
    with pytest.raises(RuntimeError):
        conn.redshift_execute_insert(table=table_name)

# test_redshift_execute_delete
def test_querying_delete_from_redshift_without_specifying_any_conditions(
    conn):
...
def test_querying_delete_from_redshift_using_specified_conditions_but_no_reference_table(
    ...
    data_after_deleted = conn.redshift_execute_delete(
        table=table_name,
        conditions="catid between 0 and 9"
    )
...
def test_querying_delete_from_redshift_using_specified_conditions_from_a_reference_table(
    ...
    expected_data = correct_return_data_when_select_query[
        'after_conditional_delete_referencing_other_table']
    assert sorted(data_after_deleted) == sorted(expected_data)
