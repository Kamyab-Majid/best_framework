...
def non_existing_object():
    return {"Bucket": bucket_name, "Key": "non_existing_obj.txt"}


@pytest.fixture
def existing_object_to_copy():
    return {"Bucket": bucket_name, "Key": "flavors.csv"}


@pytest.fixture
def copy_to_destination():
    return {"Bucket": bucket_name, "Key": "copy-destination/flavors.csv"}
...
def existing_object_to_delete():
    return {"Bucket": bucket_name, "Key": "text.txt"}


@pytest.fixture
def tag_set():
    return [{"Key": "tag_key_1", "Value": "tag_value_1"}, {"Key": "tag_key_2", "Value": "tag_value_2"}]
...
def test_put_object_to_non_existing_bucket_should_raise_runtime_error(s3_ops, json_file, upload_file_name):
    with pytest.raises(RuntimeError):
...
def test_direct_write_into_s3_bucket(s3_ops, upload_direct_file_name):
    s3_ops.write_to_s3_from_local(
        bucket_name, "tests/test_files/s3_test_files/enterprise_survey_test_writing.csv", upload_direct_file_name
    )
    result = s3_ops.validate_object(bucket_name, upload_direct_file_name)
    assert result


def test_write_to_s3_from_non_existing_local_file_should_raise_runtime_error(s3_ops, upload_direct_file_name):
    with pytest.raises(FileNotFoundError):
        s3_ops.write_to_s3_from_local(
            bucket_name, "tests/test_files/s3_test_files/non_existing_file.csv", upload_direct_file_name
        )
...
def test_move_non_existing_object_should_raise_runtime_error(s3_ops, non_existing_object, move_to_destination):
    destination_bucket = move_to_destination["Bucket"]
    destination_key = move_to_destination["Key"]
    with pytest.raises(RuntimeError):
        s3_ops.move_object(
            source=non_existing_object, destination_bucket=destination_bucket, destination_key=destination_key
        )
...
def test_move_existing_object_should_make_object_exist_in_destination_and_not_exist_in_source_after(
    ...
    destination_key = move_to_destination["Key"]
    s3_ops.move_object(
        source=existing_object_to_move, destination_bucket=destination_bucket, destination_key=destination_key
    )
...
def test_copy_non_existing_object_should_raise_runtime_error(s3_ops, non_existing_object, copy_to_destination):
...
def test_copy_existing_object_should_make_object_exist_in_both_destination_and_source_after(
    s3_ops, existing_object_to_copy, copy_to_destination
):
    destination_bucket = copy_to_destination["Bucket"]
    destination_key = copy_to_destination["Key"]
    s3_ops.copy_object(
        copy_source=existing_object_to_copy, destination_bucket=destination_bucket, destination_key=destination_key
    )
    existing_objs = objects_list_in_bucket(s3_ops, bucket_name)
...
def test_copy_existing_object_to_a_non_existing_destination_should_still_work(s3_ops, existing_object_to_copy):
    destination_key = "non-exist-folder/flavors.csv"
    s3_ops.copy_object(
        copy_source=existing_object_to_copy, destination_bucket=bucket_name, destination_key=destination_key
    )
    existing_objs = objects_list_in_bucket(s3_ops, bucket_name)
    assert existing_object_to_copy["Key"] in existing_objs
    assert destination_key in existing_objs
...
def test_delete_non_existing_object_should_raise_runtime_error(s3_ops, non_existing_object):
    with pytest.raises(RuntimeError):
        s3_ops.delete_object(bucket=non_existing_object["Bucket"], key=non_existing_object["Key"])


def test_delete_existing_object_should_make_object_not_exist_anymore_after(s3_ops, existing_object_to_delete):
    bucket = existing_object_to_delete["Bucket"]
    key = existing_object_to_delete["Key"]
    s3_ops.delete_object(bucket=bucket, key=key)
    existing_objs = objects_list_in_bucket(s3_ops, bucket_name)
    assert key not in existing_objs
    s3_ops.write_to_s3_from_local(
        bucket=bucket_name, file_path="tests/test_files/s3_test_files/text.txt", s3_key="text.txt"
    )


# test_delete_all_objects
def test_delete_all_objects_should_make_all_objects_in_bucket_not_exist_anymore_after(s3_ops, local_files_list):
    existing_objs = objects_list_in_bucket(s3_ops, bucket_name)
    ...
    for file_name in local_files_list:
        s3_ops.write_to_s3_from_local(
            bucket=bucket_name, file_path=f"tests/test_files/s3_test_files/{file_name}", s3_key=file_name
        )
...
def test_move_all_objects_should_make_objects_not_exist_in_source_but_exist_in_destination_after(
    ...
    assert set(existing_objs_in_source_bucket_before_moved).issubset(set(existing_objs_in_destination_bucket))
    for file_name in local_files_list:
        s3_ops.write_to_s3_from_local(
            bucket=bucket_name, file_path=f"tests/test_files/s3_test_files/{file_name}", s3_key=file_name
        )
...
def test_move_all_objects_from_a_non_existing_bucket_should_raise_runtime_error(s3_ops):
    destination_bucket = bucket_name + "-copied"
    with pytest.raises(RuntimeError):
        s3_ops.move_all_objects(bucket="non_existing_bckt", destination_bucket=destination_bucket)


def test_move_all_objects_to_a_non_existing_destination_bucket_should_raise_runtime_error(s3_ops):
...
def test_copy_all_objects_should_make_objects_exist_in_both_source_bucket_and_in_destination_after(s3_ops):
    destination_bucket = bucket_name + "-copied"
    ...
    s3_ops.copy_all_objects(bucket=bucket_name, destination_bucket=destination_bucket)
    existing_objs_in_source_bucket_after_copied = objects_list_in_bucket(s3_ops, bucket_name)
    assert existing_objs_in_source_bucket_after_copied == existing_objs_in_source_bucket_before_copied
    existing_objs_in_destination_bucket = objects_list_in_bucket(s3_ops, destination_bucket)
    assert set(existing_objs_in_source_bucket_before_copied).issubset(set(existing_objs_in_destination_bucket))


def test_copy_all_objects_from_a_non_existing_bucket_should_raise_runtime_error(s3_ops):
    destination_bucket = bucket_name + "-copied"
    with pytest.raises(RuntimeError):
        s3_ops.copy_all_objects(bucket="non_existing_bckt", destination_bucket=destination_bucket)


def test_copy_all_objects_to_a_non_existing_destination_bucket_should_raise_runtime_error(s3_ops):
    destination_bucket = "non_existing_bckt"
    with pytest.raises(RuntimeError):
        s3_ops.copy_all_objects(bucket=bucket_name, destination_bucket=destination_bucket)
...
def test_set_object_tag_should_make_tag_exist_in_object(s3_ops, existing_object_to_copy, tag_set):
    s3_ops.set_object_tag(bucket=bucket_name, key=existing_object_to_copy["Key"], tag_set=tag_set)
    existing_tags_list_in_obj = s3_ops.get_object_tag(bucket=bucket_name, key=existing_object_to_copy["Key"])["TagSet"]
    assert tag_set == existing_tags_list_in_obj


def test_set_object_tag_on_non_existing_object_should_raise_runtime_error(s3_ops, non_existing_object):
    with pytest.raises(RuntimeError):
        s3_ops.set_object_tag(bucket=bucket_name, key=non_existing_object["Key"], tag_set=tag_set)
...
def test_set_object_tag_without_passing_appropriate_tag_set_in_list_should_raise_type_error(
    ...
    with pytest.raises(TypeError):
        s3_ops.set_object_tag(bucket=bucket_name, key=existing_object_to_copy["Key"], tag_set="invalid tag set")
...
def test_add_tag_with_valid_key_and_value_should_create_tag_for_that_object_appropriately(
    ...
    s3_ops.add_object_tag(bucket=bucket_name, key=existing_object_to_copy["Key"], new_tag=new_tag)
    existing_tags_list_in_obj = s3_ops.get_object_tag(bucket=bucket_name, key=existing_object_to_copy["Key"])["TagSet"]
    assert new_tag in existing_tags_list_in_obj


def test_add_tag_that_has_existing_key_should_raise_value_error(s3_ops, existing_object_to_copy, new_tag):
    existing_tags_list_in_obj = s3_ops.get_object_tag(bucket=bucket_name, key=existing_object_to_copy["Key"])["TagSet"]
...
def test_add_tag_with_new_tag_passed_in_is_not_dict_type_should_raise_type_error(s3_ops, existing_object_to_copy):
    with pytest.raises(TypeError):
        s3_ops.add_object_tag(bucket=bucket_name, key=existing_object_to_copy["Key"], new_tag="invalid tag")


def test_add_tag_that_does_not_have_appropriate_key_pair_value_should_raise_value_error(
    s3_ops, existing_object_to_copy
):
    with pytest.raises(ValueError):
...
# test_get_object_tag
def test_get_tag_for_non_existing_object_should_raise_runtime_error(s3_ops, non_existing_object):
    with pytest.raises(RuntimeError):
        s3_ops.get_object_tag(bucket=bucket_name, key=non_existing_object["Key"])


def test_get_object_tag_where_there_is_no_tag_should_return_empty(s3_ops, existing_object_to_move):
    current_tags = s3_ops.get_object_tag(bucket=bucket_name, key=existing_object_to_move["Key"])
...
def test_delete_non_exisiting_tag_should_raise_runtime_error(s3_ops, existing_object_to_move, new_tag):
    with pytest.raises(RuntimeError):
        s3_ops.delete_object_tag(bucket=bucket_name, key=existing_object_to_move["Key"], tag=new_tag)
...
def test_delete_tag_that_does_not_have_appropriate_key_value_pair_should_raiseruntime_error(
    ...
    with pytest.raises(RuntimeError):
        s3_ops.delete_object_tag(
            bucket=bucket_name, key=existing_object_to_move["Key"], tag={"WrongKey": "tag_key_1", "Value": "tag_val_2"}
        )


def test_delete_tag_that_is_not_passed_in_as_dict_type_should_raise_runtime_error(s3_ops, existing_object_to_move):
    with pytest.raises(RuntimeError):
        s3_ops.delete_object_tag(bucket=bucket_name, key=existing_object_to_move["Key"], tag="not valid tag")


def test_delete_existing_tag_should_make_tag_not_exist_in_object_anymore(s3_ops, existing_object_to_copy):
    tag_to_delete = {"Key": "tag_key_1", "Value": "tag_value_1"}
    existing_tags_list_in_obj = s3_ops.get_object_tag(bucket=bucket_name, key=existing_object_to_copy["Key"])["TagSet"]
    assert tag_to_delete in existing_tags_list_in_obj

    s3_ops.delete_object_tag(bucket=bucket_name, key=existing_object_to_copy["Key"], tag=tag_to_delete)
    existing_tags_list_in_obj_after_delete_tag = s3_ops.get_object_tag(
        bucket=bucket_name, key=existing_object_to_copy["Key"]
    )["TagSet"]
...
def test_set_bucket_tag_should_make_tag_exist_in_bucket(s3_ops, tag_set):
    ...
    existing_tags_list_in_bckt = s3_ops.get_bucket_tag(bucket=bucket_name)["TagSet"]
...
def test_set_bucket_tag_without_passing_appropriate_tag_set_in_list_should_raise_error(s3_ops):
    with pytest.raises(ValueError):
        s3_ops.set_bucket_tag(bucket=bucket_name, tag_set=[{"WrongKey": "tag_key_1", "Value": "tag_val_2"}])

    with pytest.raises(TypeError):
        s3_ops.set_bucket_tag(bucket=bucket_name, tag_set="invalid tag set")
...
def test_delete_bucket_tag_should_make_bucket_have_no_tags(s3_ops, tag_set):
    ...
    existing_tags_list_in_bckt_after_delete_tag = s3_ops.get_bucket_tag(bucket=bucket_name)["TagSet"]
